...
- Fixed Tauri v2.11 Rust compile errors by enabling the `tray-icon` feature, using `set_focus()`, dropping backend process locks before async health checks, and removing the invalid tray fallback icon constructor.
